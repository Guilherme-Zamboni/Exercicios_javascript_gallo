...
class EditorSimplesDeImagem:
    def __init__(self,janela):
        self.janela = janela
        self.janela.title('Editor Simples de Imagem')
    ...
    def criar_menu(self):
        ...
        menu_arquivo = tk.Menu(self.menu, tearoff = 0)
    ...
    def novo_arquivo(self):
        if self.modificado and messagebox.askyesno("Salvar?", "Deseja salvar o desenho atual?"):
            self.salvar_arquivo()
            self.imagem = Image.new('RGB', (900,600), 'white')
            self.desenho = ImageDraw.Draw(self.imagem)
            self.atualizar_area_desenho()
